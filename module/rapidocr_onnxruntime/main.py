...
class RapidOCR:
    ...
    def get_crop_img_list(
        ...
        def warp_perspective(img, H, width, height):
            H_inv = np.linalg.inv(H)
            u, v = np.meshgrid(np.arange(width), np.arange(height))
            uv1 = np.stack([u, v, np.ones_like(u)], -1)
            xy = uv1 @ H_inv.T
            x = (xy[..., 0] / xy[..., 2]).clip(0, img.shape[1]-1)
            y = (xy[..., 1] / xy[..., 2]).clip(0, img.shape[0]-1)
            
            x0, y0 = np.floor(x).astype(int), np.floor(y).astype(int)
            x1, y1 = x0 + 1, y0 + 1
            x1 = x1.clip(0, img.shape[1]-1)
            y1 = y1.clip(0, img.shape[0]-1)
        ...
        def get_rotate_crop_image(img: np.ndarray, points: np.ndarray) -> np.ndarray:
            ...
            H = compute_homography(points, dst_pts)
            warped = warp_perspective(img, H, img_crop_width, img_crop_height)
        ...
        for box in dt_boxes:
            ...
            img_crop = get_rotate_crop_image(img, copy.deepcopy(tmp_box))
